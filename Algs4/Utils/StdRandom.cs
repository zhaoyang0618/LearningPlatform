...
namespace Algs4.Utils
{
    ...
    public static class StdRandom
    {
        ...
        private static long _seed;        // pseudo-random number generator seed
        ...
        {
            if (n <= 0) throw new InvalidDataException("argument must be positive: " + n);
        ...
        {
            if (n <= 0L) throw new InvalidDataException("argument must be positive: " + n);
        ...
        public static int uniformInt(int a, int b)
        {
            ...
            {
                throw new InvalidDataException("invalid range: [" + a + ", " + b + ")");
            }
        ...
        {
            if (!(p >= 0.0 && p <= 1.0))
                throw new InvalidDataException("probability p must be between 0.0 and 1.0: " + p);
        ...
        public static int geometric(double p)
        {
            if (!(p >= 0))
            {
                throw new InvalidDataException("probability p must be greater than 0: " + p);
            }
            if (!(p <= 1.0))
            {
                throw new InvalidDataException("probability p must not be larger than 1: " + p);
            }
            // using algorithm given by Knuth
            return (int)Math.Ceiling(Math.Log(uniformDouble()) / Math.Log(1.0 - p));
        }
        ...
        {
            if (!(lambda > 0.0))
                throw new InvalidDataException("lambda must be positive: " + lambda);
            if (Double.IsInfinity(lambda))
                throw new InvalidDataException("lambda must not be infinite: " + lambda);
            // using algorithm given by Knuth
        ...
        {
            if (!(alpha > 0.0))
                throw new InvalidDataException("alpha must be positive: " + alpha);
        ...
        {
            if (probabilities == null) throw new InvalidDataException("argument array must not be null");
            double EPSILON = 1.0E-14;
            double sum = 0.0;
            for (int i = 0; i < probabilities.Length; i++)
            {
                if (!(probabilities[i] >= 0.0))
                    throw new InvalidDataException("array entry " + i + " must be non-negative: " + probabilities[i]);
                sum += probabilities[i];
            }
            if (sum > 1.0 + EPSILON || sum < 1.0 - EPSILON)
                throw new InvalidDataException("sum of array entries does not approximately equal 1.0: " + sum);
        ...
        {
            if (frequencies == null) throw new InvalidDataException("argument array must not be null");
            long sum = 0;
            for (int i = 0; i < frequencies.Length; i++)
            {
                if (frequencies[i] < 0)
                    throw new InvalidDataException("array entry " + i + " must be non-negative: " + frequencies[i]);
                sum += frequencies[i];
            }
            if (sum == 0)
                throw new InvalidDataException("at least one array entry must be positive");
            if (sum >= int.MaxValue)
                throw new InvalidDataException("sum of frequencies overflows an int");
        ...
        {
            if (n < 0) throw new InvalidDataException("n must be non-negative: " + n);
            int[] perm = new int[n];
            for (int i = 0; i < n; i++)
                perm[i] = i;
            shuffle(perm);
        ...
        {
            if (n < 0) throw new InvalidDataException("n must be non-negative: " + n);
            if (k < 0 || k > n) throw new InvalidDataException("k must be between 0 and n: " + k);
            int[] perm = new int[k];
            for (int i = 0; i < k; i++)
            {
        ...
        private static void validateSubarrayIndices(int lo, int hi, int length)
        {
            ...
            {
                throw new InvalidDataException("subarray indices out of bounds: [" + lo + ", " + hi + ")");
            }
        }
        ...
        public static void test(String[] args)
        {
            ...
            if (args.Length == 2) StdRandom.setSeed(long.Parse(args[1]));
            double[] probabilities = { 0.5, 0.3, 0.1, 0.1 };
            int[] frequencies = { 5, 3, 1, 1 };
            String[] a = "A B C D E F G".Split(" ");

            System.Console.WriteLine("seed = " + StdRandom.getSeed());
            for (int i = 0; i < n; i++)
            {
                System.Console.WriteLine("%2d ", uniformInt(100));
                System.Console.WriteLine("%8.5f ", uniformDouble(10.0, 99.0));
                System.Console.WriteLine("%5b ", bernoulli(0.5));
                System.Console.WriteLine("%7.5f ", gaussian(9.0, 0.2));
                System.Console.WriteLine("%1d ", discrete(probabilities));
                System.Console.WriteLine("%1d ", discrete(frequencies));
                System.Console.WriteLine("%11d ", uniformLong(100000000000L));
                StdRandom.shuffle(a);
